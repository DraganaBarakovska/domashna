...
let studentAverageGrade = students.filter(x => x.averageGrade === 5).map(x => `${x.firstName} ${x.lastName}`)
  .forEach(x => console.log(x));



let MaleStudent = students.filter(x => x.gender === "Male" && x.city === "Skopje" && x.age > 18).map(x => `${x.firstName} ${x.lastName}`)
  .forEach(x => console.log(x));


let FemaleAge = students.filter(x => x.gender === "Female" && x.age > 24).map(x => `${x.firstName} ${x.lastName} ${x.averageGrade}`)
  .forEach(x => console.log(x));
let Grade = students.filter(x => x.averageGrade > 24).map(x => `${x.averageGrade}`)
  .forEach(x => console.log(x));



let = StudentsSort = students.map(x => `${x.lastName}`)
console.log(StudentsSort);
StudentsSort.sort((a, b) => a.localeCompare(b, "fr", { ignorePunctuation: true }));
console.log(StudentsSort);